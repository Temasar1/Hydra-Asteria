...
const consumeAsteria = async (asteriaUtxo) => {
    ...
    const adminUTxOs = await myWallet.getUtxos()
        .then((us) => us.filter((u) => u.output.amount.find((Asset) => Asset.unit === adminTokenUnit)));
    ...
    const unsignedTx = await txBuilder
        ...
        .spendingPlutusScriptV3()
        .txIn(adminUtxo.input.txHash, adminUtxo.input.outputIndex, adminUtxo.output.amount, adminUtxo.output.address)
        .txInRedeemerValue(consumeRedeemer, "JSON")
        .spendingTxInReference(asteriaDeployScript.txHash, 0)
