...
import { conStr1, deserializeDatum, MeshTxBuilder, stringToHex } from "@meshsdk/core";
import { readFile } from "fs/promises";
const changeAddress = await myWallet.getChangeAddress();
const collateral = (await myWallet.getCollateral())[0];
const utxos = await myWallet.getUtxos();
async function quit(ship_tx_hash) {
    ...
    const pellet_plutus_script = pellet_scriptref;
    ...
    const shipInputFuel = ship.output.amount.find((Assets) => Assets.unit == shipyard_policyid + fuelTokenName);
    ...
    const ship_datumLastMoveLatestTime = shipInputDatum[4].int;
