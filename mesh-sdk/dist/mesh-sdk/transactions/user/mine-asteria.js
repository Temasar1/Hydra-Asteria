...
async function mineAsteria(max_asteria_mining, ship_tx_hash, tx_earliest_posix_time) {
    ...
    const shipInputFuel = ship.output.amount.find((Asset) => Asset.unit == fuelPolicyid + fuelTokenName);
    const shipFuel = shipInputFuel?.quantity;
    const rewardAsset = asteria.output.amount.find((Asset) => Asset.unit === "Lovelace");
