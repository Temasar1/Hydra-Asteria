import { byteString, conStr0, conStr1, deserializeDatum, integer, MeshTxBuilder, policyId, serializePlutusScript, stringToHex } from "@meshsdk/core";
...
async function gatherFuel(gather_amount, ship_tx_hash, pellet_tx_Hash, tx_earliest_posix_time) {
    ...
    const pelletPlutusScript = pelletScriptRef;
    ...
    const ShipPosX = shipInputDatum[0].int;
    const shipPoxY = shipInputDatum[1].int;
    const shipTokenName = shipInputDatum[2].bytes;
    const pilotTokenName = shipInputDatum[3].bytes;
    const lastMoveLatestTime = shipInputDatum[4].bytes;
    //Ship output Datum
    ...
    const pelletPosX = pelletInputDatum[0].int;
    const pelletPosY = pelletInputDatum[1].int;
    const pelletInputShipyardPolicy = pelletInputDatum[2].bytes;
    //pellet output Datum
    ...
    const shipInputFuelAsset = ship.output.amount.find((asset) => asset.unit === fueltokenUnit);
    const pelletInputFuelAsset = pellet.output.amount.find((asset) => asset.unit === fueltokenUnit);
    const pelletFuel = pelletInputFuelAsset?.quantity;
    const shipFuel = shipInputFuelAsset?.quantity;
    const spacetimeOutputAssets = [{
    ...
    const unsignedTx = await txBuilder
        ...
        .spendingPlutusScriptV3()
        .txIn(pellet.input.txHash, pellet.input.outputIndex, pellet.output.amount, pellet.output.address)
